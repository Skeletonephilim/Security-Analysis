...
Wika is a pre-sales engineer. He picked “workflow tools” the way operators pick payloads — fast, convenient, under-validated. An unknown adversary sabotaged his targets on a Mac. We got a Cedarpelta live-response triage (`Gatekeeper.zip`, password `hacktheblue`), not a full disk/memory image. DiskImage/ and Memory/ are empty placeholders. Juice is in Unified Logs, bash history, Safari, quarantine strings, and `lsregister`.

Host at collection: `wikas-Mac`, macOS `10.13.6` (17G66), user `wika` (501). Collection stamp `2025-02-04 02:26 EET` from `~/Downloads/Mac_Live_Response`.

```bash
> 7z x -p'hacktheblue' Gatekeeper.zip -oGatekeeper-analysis -y
...
1. **Bash history left intact** — the entire post-exploitation script is still in `User-wika-bash_History.txt` / `.bash_sessions`. On macOS that is free attribution of intent. Clear history, shred sessions, or never use an interactive logged shell (`HISTFILE=/dev/null`, memory-only, etc.).
...
Blue methodology that actually found the juice

Do not start from “malware sample.” Start from operator artifacts:
...
Scar for purple: if your cleanup is `rm -r priv` but you leave history, taunts, and the victim’s missing livelihood, you did not clean up — you scheduled the IR call.
